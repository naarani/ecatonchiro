...
import org.naarani.ecantonchiro.jclouds.digitalocean.ProvisionDigitalOcean;
...
public class AMainKube {
	...
	static public void main( int size ) throws RunNodesException, IOException, StopAction {
		ProvisionDigitalOcean drop = new ProvisionDigitalOcean();
		Set<? extends NodeMetadata> nodi = drop.buildDroplet( null, SERVER_DISTRO, size, rootPwd, SERVER_LABEL, new File( workdir )
				, new File( "private" ) );
		...
		String rootUser = null; // XXX drop.getDefaultUser();
		if( !fileAnsi.exists() ){
